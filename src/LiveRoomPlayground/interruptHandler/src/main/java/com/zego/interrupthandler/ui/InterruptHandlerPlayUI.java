...
import com.zego.interrupthandler.databinding.ActivityInterruptHandlerPlayBinding;
...
public class InterruptHandlerPlayUI extends BaseActivity {


    private ActivityInterruptHandlerPlayBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_interrupt_handler_play);
        ...
        ZGPlayHelper.sharedInstance().setPlayerCallback(new IZegoLivePlayerCallback() {
            ...
            public void onPlayStateUpdate(int errorCode, String streamID) {
                ...
                if (errorCode == 0) {
                    ...
                    Toast.makeText(InterruptHandlerPlayUI.this, getString(com.zego.common.R.string.tx_play_success), Toast.LENGTH_SHORT).show();
                    ...
                    Toast.makeText(InterruptHandlerPlayUI.this, getString(com.zego.common.R.string.tx_play_fail), Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onPlayQualityUpdate(String s, ZegoPlayStreamQuality zegoPlayStreamQuality) {
                 ...
                 */
                streamQuality.setFps(String.format(Locale.CHINA, "帧率: %f", zegoPlayStreamQuality.vdjFps));
                streamQuality.setBitrate(String.format(Locale.CHINA, "码率: %f kbs", zegoPlayStreamQuality.vkbps));
            }
            ...
            public void onVideoSizeChangedTo(String streamID, int width, int height) {
                ...
                streamQuality.setResolution(String.format(Locale.CHINA, "分辨率: %dX%d", width, height));
            }
        });
                ...
                for (ZegoStreamInfo streamInfo : zegoStreamInfos) {
                    if (streamInfo.streamID.equals(mStreamID)) {
                        if (type == ZegoConstants.StreamUpdateType.Added) {
                            ...
                            Toast.makeText(InterruptHandlerPlayUI.this, com.zego.common.R.string.tx_current_stream_delete, Toast.LENGTH_LONG).show();
                            // 修改标题状态拉流成功状态
    ...
    public void startPlay() {
        ...
        ZGBaseHelper.sharedInstance().loginRoom(mRoomID, ZegoConstants.RoomRole.Audience, new IZegoLoginCompletionCallback() {
            ...
            public void onLoginCompletion(int errorCode, ZegoStreamInfo[] zegoStreamInfos) {
                // 关闭dialog
                CustomDialog.createDialog(InterruptHandlerPlayUI.this).cancel();
                if (errorCode == 0) {
                    AppLogger.getInstance().i(InterruptHandlerPlayUI.class, "登陆房间成功 roomId : %s", mRoomID);

                    // 开始拉流
                    ZGPlayHelper.sharedInstance().startPlaying(mStreamID, binding.playView);
                } else {
                    AppLogger.getInstance().i(InterruptHandlerPlayUI.class, "登陆房间失败, errorCode : %d", errorCode);
                    Toast.makeText(InterruptHandlerPlayUI.this, getString(com.zego.common.R.string.tx_login_room_failure), Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    public static void actionStart(Activity activity, String roomID, String streamID) {
        Intent intent = new Intent(activity, InterruptHandlerPlayUI.class);
